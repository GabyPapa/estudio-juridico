...
:: 4. Crear base de datos via sqlcmd (si está disponible)
echo.
echo [4/4] Creando base de datos EstudioJuridico...
where sqlcmd >nul 2>&1
IF %ERRORLEVEL% EQU 0 (
    sqlcmd -S localhost\SQLEXPRESS -U sa -P Admin2024! -Q "IF NOT EXISTS (SELECT name FROM sys.databases WHERE name='EstudioJuridico') CREATE DATABASE EstudioJuridico COLLATE Modern_Spanish_CI_AI; PRINT 'DB lista'" -b
    IF %ERRORLEVEL% EQU 0 (
        echo [OK] Base de datos verificada
        echo.
        echo [EJECUTAR SQL] Ahora abre SSMS y ejecuta en orden:
    ...
    echo [INFO] sqlcmd no encontrado. Ejecuta los scripts SQL manualmente en SSMS.
)
